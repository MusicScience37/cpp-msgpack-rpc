...
/*!
 * \file
 * \brief Definition of UnixSocketAcceptorFactory class.
 */
#pragma once
...
/*!
 * \brief Class of factory to create acceptors of Unix sockets.
 */
class UnixSocketAcceptorFactory final : public IAcceptorFactory {
public:
    // Change to template when another protocol is implemented.
     ...
     */
    UnixSocketAcceptorFactory(std::shared_ptr<executors::IExecutor> executor,
    ...
    std::vector<std::shared_ptr<IAcceptor>> create(
        ...
        const ConcreteAddress local_address(uri.host_or_file_path());
