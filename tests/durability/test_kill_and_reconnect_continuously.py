"""Test to kill and reconnect clients continuously."""
...
CLIENT_EXECUTION_TIME = datetime.timedelta(seconds=10)
CLIENT_KILL_TIME = datetime.timedelta(seconds=1)


def _check_kill_and_reconnect_continuously(
    *,
    ...
    with ProcessExecutor(
        ...
        while datetime.datetime.now() < finish_time:
            with ProcessExecutor(
                ...
                time.sleep(CLIENT_KILL_TIME.total_seconds())
                client_process.stop()

    assert server_process.returncode == 0


def test_kill_and_reconnect_continuously_tcp_v4(
    bin_dir_path: pathlib.Path,
    log_parent_dir_path: pathlib.Path,
    test_duration: datetime.timedelta,
) -> None:
    """Test to kill and reconnect clients continuously (TCPv4)."""
    current_log_dir_path = (
        log_parent_dir_path / "test_kill_and_reconnect_continuously_tcp_v4"
    )
    ...
    _check_kill_and_reconnect_continuously(
...
def test_kill_and_reconnect_continuously_unix(
    bin_dir_path: pathlib.Path,
    log_parent_dir_path: pathlib.Path,
    test_duration: datetime.timedelta,
) -> None:
    """Test to kill and reconnect clients continuously (Unix socket)."""
    current_log_dir_path = (
        log_parent_dir_path / "test_kill_and_reconnect_continuously_unix"
    )
    if current_log_dir_path.exists():
        shutil.rmtree(str(current_log_dir_path))
    current_log_dir_path.mkdir(exist_ok=True, parents=True)
