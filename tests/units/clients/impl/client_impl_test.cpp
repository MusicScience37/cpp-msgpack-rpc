...
TEST_CASE("msgpack_rpc::clients::impl::ClientImpl") {
    ...
    const std::shared_ptr<IClientImpl> client = std::make_shared<ClientImpl>();

    SECTION("asynchronously call a method") {
        ...
        const auto serialized_request = MessageSerializer::serialize_request(
            method_name, request_id, param1);

        post([&client, &method_name, &serialized_request] {
            const std::shared_ptr<ICallFutureImpl> future =
                client->async_call(method_name, request_id, serialized_request);
        });
